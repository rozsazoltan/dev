...
fn create_formats_detected_disk_detaches_then_returns_registry_record() {
    let plan = CreatePlan { path: PathBuf::from(r"D:\Dev\disks\projects.vhdx"), capacity_bytes: 68_719_476_736, dynamic: true };
    let mut process = FakeProcess { responses: VecDeque::from([
        Ok(String::new()),
        Ok(r#"{"blockdevices":[{"name":"sda","type":"disk"}]}"#.into()),
        Ok(String::new()),
        Ok(r#"{"blockdevices":[{"name":"sda","type":"disk"},{"name":"sdd","type":"disk"}]}"#.into()),
        Ok(String::new()),
        Ok(String::new()),
    ]), calls: vec![] };

    let registry = std::env::temp_dir().join(format!("wsldisk-workflow-{}.json", std::process::id()));
...
fn formatting_failure_still_detaches_and_never_returns_a_disk() {
    let plan = CreatePlan { path: PathBuf::from(r"D:\Dev\disks\projects.vhdx"), capacity_bytes: 68_719_476_736, dynamic: true };
    let mut process = FakeProcess { responses: VecDeque::from([
        Ok(String::new()), Ok(r#"{"blockdevices":[]}"#.into()), Ok(String::new()),
        Ok(r#"{"blockdevices":[{"name":"sdd","type":"disk"}]}"#.into()),
        Err(io::Error::other("mkfs failed")), Ok(String::new()),
    ]), calls: vec![] };

    let registry = std::env::temp_dir().join(format!("wsldisk-failure-{}.json", std::process::id()));
